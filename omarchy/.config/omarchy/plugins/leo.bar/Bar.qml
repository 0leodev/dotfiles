...
Item {
  ...
  // plugin bar is loaded (Quickshell's async plugin Loader cannot satisfy
  // `required` properties at construction), so it is declared with a default
  // here and overwritten by configureBar before it is ever read.
  ...
  // Desired on-screen bar width in px. 0 (or unset) means the bar spans the
  // whole screen edge. Positive values render a floating bar centered on the
  // edge with transparent margins either side.
  property int barWidth: 0
  // Radius for the bottom corners of the (non-transparent) bar background, in
  // px. Independent of the system corner radius so a floating bar can have
  // only its outer corners rounded.
  ...
  function applyBarConfig() {
    ...
    barWidth = Math.max(0, Number(config.width) || 0)
    bottomRadius = Math.max(0, Number(config["bottom-radius"]) || 0)
  ...
  component BarPanel: PanelWindow {
    ...
    // The bar background (including the floating-width and per-corner radius
    // cases) is drawn by a dedicated rounded shape inside the window, so the
    // window itself is always transparent.
    color: "transparent"
    surfaceFormat.opaque: false
    WlrLayershell.namespace: "omarchy-bar"
    WlrLayershell.layer: WlrLayer.Top

    // Horizontal (top/bottom) bars can be floated to a fixed width and
    // horizontally centered by setting `bar.width`. The content (widgets) and
    // the rounded background both live inside this centered, width-constrained
    // container so transparent margins appear on either side. Vertical bars
    // always fill the (already thin) surface.
    Item {
      id: barSurfaceContent
      x: root.vertical ? 0 : Math.round((parent.width - width) / 2)
      y: root.vertical ? Math.round((parent.height - height) / 2)
         : (root.position === "bottom" ? parent.height - height : 0)
      width: root.vertical ? parent.width : (root.barWidth > 0 ? root.barWidth : parent.width)
      height: root.vertical ? parent.height : parent.height

      // Rounded, width-constrained background drawn only for horizontal bars.
      Loader {
        active: !root.vertical
        visible: !root.transparent
        sourceComponent: barBackgroundShape
        anchors.fill: parent
      }
    ...
    Component {
      ...
      // Rectangle fill for the bar with a configurable bottom-corner radius.
      // Qt Quick's Rectangle only supports a uniform radius, so the bottom
      // rounding is drawn with a Shape/path that keeps the top corners square.
      Item {
        id: backgroundHost
        anchors.fill: parent

        readonly property int availRadius: Math.min(root.bottomRadius, Math.floor(Math.min(width, height) / 2))

        Shape {
          anchors.fill: backgroundHost
          antialiasing: true
          smooth: true
          ...
          ShapePath {
            ...
            PathLine { x: backgroundHost.width; y: 0 }
            PathLine { x: backgroundHost.width; y: backgroundHost.height - backgroundHost.availRadius }
            PathArc {
              x: backgroundHost.width - backgroundHost.availRadius
              y: backgroundHost.height
              radiusX: backgroundHost.availRadius
              radiusY: backgroundHost.availRadius
            }
            PathLine { x: backgroundHost.availRadius; y: backgroundHost.height }
            PathArc {
              x: 0
              y: backgroundHost.height - backgroundHost.availRadius
              radiusX: backgroundHost.availRadius
              radiusY: backgroundHost.availRadius
            }
            PathLine { x: 0; y: 0 }
